...
export class Database {
    static async getPiece(_id: number): Promise<Piece | undefined> {
        return undefined;
    }

    static async getPieceAuthorId(name: string): Promise<number> {
        const existingId = await Database.getExistingAuthorId(name);
    ...
    static async getPieceTagId(tag: string): Promise<number> {
        const existingId = await Database.getExistingTagId(tag);
    ...
    public static async addPieceTags(pieceId: number, tags: string[]): Promise<void> {
        const ids = await Promise.all(tags.map(tag => Database.getPieceTagId(tag)));
        await Promise.all(ids.map(id => pool.query(
            'insert into piece_tags(piece_id, tag_id) values ($1, $2)',
            [pieceId, id])));
    }

    static async addPiece(piece: Piece, userId: number): Promise<number> {
        const authorId = piece.author !== undefined ?
            await Database.getPieceAuthorId(piece.author) : null;
        ...
        await Database.addPieceTags(res.rows[0].id, piece.tags);

        return Promise.resolve(res.rows[0].id);
    }

    static async getNotificationId(pieceId: number): Promise<number> {
        return Promise.resolve(pieceId);
    }

    public static async getPiecesMeta(userId: number): Promise<PieceBase[]> {
    ...
    static async toggleIsFavourite(id: number): Promise<void> {
        await pool.query('update piece set is_favourite = not is_favourite where id = $1',
            [id]);
    }

    static async updatePiece(piece: Piece): Promise<void> {
    ...
    static async createUser(email: string, password: string): Promise<number> {
    ...
    static async findUserPieceByName(name: string, userId: number): Promise<Piece | undefined> {
    ...
    public static async findUserPieceById(id: number, userId: number): Promise<Piece | undefined> {
        const res = await pool.query(
    ...
    public static async deletePiece(id: number) {
    ...
    private static async getExistingAuthorId(name: string): Promise<number | undefined> {
    ...
    private static async getExistingTagId(tag: string): Promise<number | undefined> {
